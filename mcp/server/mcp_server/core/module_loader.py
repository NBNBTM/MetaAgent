...
class ModuleLoader:
    ...
    def _find_module_class(self, module_name):
        ...
        # 如果主模块中没有找到，尝试导入子模块
        try:
            sub_module = importlib.import_module(f"modules.{module_name}.{module_name}")
            ...
            print(f"警告: 导入子模块 {module_name}.{module_name} 失败: {e}")
        ...
        for item in (self.modules_dir / module_name).iterdir():
            ...
            try:
                ...
                print(f"警告: 导入子模块 {module_name}.{sub_module_name} 失败: {e}")
                continue

        print(f"错误: 在模块 {module_name} 中未找到有效的模块类")
        return None
