...
class Calculator(ModuleInterface):
    ...
    def get_info(self):
        ...
        return {
            "name": "Calculator",
            "version": "1.0.0",
            "description": "基础计算功能，实现四则运算，表达式计算，也可计算均值、众数、标准差、导数、不定积分、皮尔逊相关系数、线性回归、矩阵加法、矩阵乘法、矩阵转置等。",
            "author": "Lindsey"
        }
    ...
    def register(self, server):
        ...
        def confidence_interval(data: List[float], confidence: float = 0.95) -> dict:
            ...
            try:
                ...
                return {
                    "confidence_interval": (float(mean_value - margin_of_error), float(mean_value + margin_of_error))}
            except Exception as e:
                return {"error": str(e)}
